...
updater() {
  ...
  cloudShellCode=`curl "https://malhadijr.com/resource/applevalid/$aapl" -s | sha256sum`

  localCookies=`cat cookies.txt | sha256sum`
  cloudCookies=`curl "https://malhadijr.com/resource/applevalid/cookies.txt" -s | sha256sum`

  localJsCode=`cat netlog.js | sha256sum`
  cloudJsCode=`curl "https://malhadijr.com/resource/applevalid/netlog.js" -s | sha256sum`

  if [[ $localShellCode != $cloudShellCode || $localCookies != $cloudCookies || $localJsCode != $cloudJsCode ]]; then
    echo "Updating script... Please wait."
    rm -f $0; wget --quiet "https://malhadijr.com/resource/applevalid/$aapl"; mv $aapl $0; chmod +x $0
    rm -f cookies.txt; wget --quiet "https://malhadijr.com/resource/applevalid/cookies.txt"; chmod 755 cookies.txt
    rm -f netlog.js; wget --quiet "https://malhadijr.com/resource/applevalid/netlog.js"; chmod 755 netlog.js
...
echo "[+] Doing an automatic update from server malhadijr.com on `date`"
updater "auto"
